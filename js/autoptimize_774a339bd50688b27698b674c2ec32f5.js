...
function initDarkMode(){let darkModeID=foxizCoreParams.darkModeID;let currentMode=localStorage.getItem(darkModeID);if(null===currentMode){currentMode=document.body.getAttribute('data-theme');}
...
self._body.addClass('switch-smooth');if(null===currentMode||'default'===currentMode){localStorage.setItem(darkModeID,'dark');setDarkModeCookie(darkModeID,'dark');self._body.attr('data-theme','dark');iconDefault.removeClass('activated');iconDark.addClass('activated');}else{localStorage.setItem(darkModeID,'default');setDarkModeCookie(darkModeID,'default');self._body.attr('data-theme','default');iconDefault.addClass('activated');iconDark.removeClass('activated');}})}
...
Module.init=function(){this.initParams();this.followToggle();this.syncBookmark();this.myBookmarks();this.topSpacing();this.stickyNavBar();this.stickyHeaderBuilder();this.siteAccessDetector();this.headerDropdown();this.mobileCollapse();this.privacyTrigger();this.popupNewsletter();this.backTop();this.hoverEffects();this.paginationInfinite();this.loadYoutubeIframe();this.singleInfiniteLoadNext();this.highlightShares();this.readIndicatorInit();this.fontResizer();this.sliders();this.carousels();this.breakingNews();this.liveSearch();this.browserResize();this.initSubMenuPos();this.footerSlideUp();this.initAjaxBlocks();this.documentReload();};Module.documentReload=function(){this.documentClick();this.bookmarkToggle();this.loginPopup();this.hoverTipsy();this.paginationNextPrev();this.paginationLoadMore();this.showPostComment();this.scrollToComment();this.replyReview();this.usersRating();this.singleGallery();this.floatingVideo();this.videoPreview();this.floatingVideoRemove();this.scrollTableContent();this.singleScrollRefresh();this.productQuantity();this.videoAutoPlay();this.tocToggle();};Module.reInitAll=function(){this.html.off();this.document.off();this.window.trigger('load');this.refreshBookmarks();this.documentReload();};Module.reInitPagination=function(){this.html.off();this.document.off();this.window.trigger('load');this.refreshBookmarks();this.paginationNextPrev();this.paginationLoadMore();this.paginationInfinite();this.bookmarkToggle();this.videoPreview();this.loginPopup();this.hoverTipsy();this.documentClick();}
...
if(window.innerWidth<=1024){if(!self.eSticky.smartSticky){if(self.body.hasClass('yes-tstick')){self.eSticky.isTitleSticky=true;self.eSticky.activatePos=self.eSticky.activatePos+400;self.eSticky.deactivePos=self.eSticky.deactivePos+400;}
...
if(infoTemplate.length){infoTemplate.fadeOut(100,function(){infoTemplate.find('.bookmark-desc').html(response.description);infoTemplate.fadeIn(300);self.timeOut=setTimeout(function(){infoTemplate.fadeOut(600);},2000);});infoTemplate.on({mouseenter:function(){clearTimeout(self.timeOut);},mouseleave:function(){self.timeOut=setTimeout(function(){infoTemplate.fadeOut(600);},1200);}});}
target.removeClass('in-progress');self.followProgressing=false;}});});};Module.initAjaxBlocks=function(){var self=this;var elements=$('.is-ajax-block');if(elements.length>0){elements.each(function(){var block=$(this);var uuid=block.attr('id');if(!self.ajaxData[uuid]){self.ajaxData[uuid]=self.getBlockSettings(uuid);}
